function [x] = gradiente_descendente_ordem_3(g, simbolos, x0, Toler, IterMax)
    ...
    while k <= IterMax
        ...
        % 3º passo:
        g1 = g_func(x(1), x(2), x(3));
        z = grad_g_func(x(1), x(2), x(3));
        z0 = norm(z,2);
        ...
        if abs(g_func(x(1), x(2), x(3)) - g1) < Toler
            % disp(x)
            break
        end
